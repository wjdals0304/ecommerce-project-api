...
export class AuthService {
  ...
  async signUpWithEmail(createUserDto: CreateUserDto, res: any) {
    try {
      ...
      res.setHeader(
        "set-cookie",
        `refreshToken=${refreshToken}; HttpOnly; Path=/; Max-Age=604800;${process.env.NODE_ENV === "production" ? " Secure; SameSite=None;" : ""}`,
      );
  ...
  async signUpWithGoogle(
    tokenDto: { access_token: string; provider: string },
    res: any,
  ) {
    try {
      ...
      const supabaseUserResponse = await fetch(
        "https://pthoxfolwfcdmudnqtag.supabase.co/auth/v1/user",
        {
          headers: {
  ...
  signInWithGoogle(user: any, res: any) {
    try {
      const payload = { email: user.email, sub: user.id };
      const accessToken = this.jwtService.sign(payload, { expiresIn: "1h" });
      const refreshToken = this.jwtService.sign(payload, { expiresIn: "7d" });
